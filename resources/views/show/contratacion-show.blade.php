        ...
        <div class="container sticky-parent">
                    ...
                    <div class="item">
                        <div class="item_main">
                            <div class="item_main-media">
                                <picture>
                                    <source data-srcset="{{ asset('storage/'.$show->image) }}" srcset="{{ asset('storage/'.$show->image) }}" type="image/webp" />
                                    <img
                                        class="lazy"
                                        data-src="{{ asset('storage/'.$show->image) }}"
                            ...
                            <div class="item_main-about">
                                ...
                                <a class="title h4">{{$show->name}}</a>
                                <span class="price h4">$ {{$show->price}}.00 MXN</span>
                                <ul class="details"> </ul>
                            </div>
                        </div>
                        <div class="item_price item_price--item">
                            <span class="property"></span>
                            <span class="value">$ {{$show->price}}.00 MXN</span>
                        </div>
            ...
            <div class="sticky-wrapper" data-sticky="true">
                <div class="cart_content-summary">
                    ...
                    <form method="post" action="{{route('checkout_shows')}}">
                        @csrf
                        ...
                        <div class="cart_content-summary_main">
                            <div class="wrapper">
                                <div class="optional d-flex">
                                    <div class="optional_block">
                                        <label class="optional_block-label" for="date">Dirección evento</label>
                                        <input type="text" class="field" placeholder="Direccion" id="address" name="address" />
                                    </div>
                                    <div class="optional_block">
                                        <input type="hidden" class="field" placeholder="Dirección" id="show_id" name="show_id" value="{{$show->id}}"/>
                                        <input type="hidden" class="field" placeholder="Direccion" id="duration" name="duration" value="{{$show->duration}}"/>
                                        <input type="hidden" class="field" placeholder="Direccion" id="price" name="price" value="{{$show->price}}"/>
                                        <input type="hidden" class="field" placeholder="Direccion" id="title" name="title" value="{{$show->name}}"/>
                                    </div>
                                </div>
                                <br>
                                <div class="optional d-flex">
                                    <div class="optional_block">
                                        <label class="optional_block-label" for="date">Fecha</label>
                                        <input type="date" class="field" placeholder="Direccion" id="date" name="date" />
                                    </div>
                                    <div class="optional_block">
                                        <label class="optional_block-label" for="email">Hora</label>
                                        <input type="time" class="field" placeholder="Correo" id="time" name="time" />
                                    </div>
                                </div>
                                <br>
